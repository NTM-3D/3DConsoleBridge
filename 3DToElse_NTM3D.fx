...
uniform int Input_Half_Full <
	ui_type = "combo";
	ui_items = "Half\0Full\0";
	ui_label = "Input Half/Full";
	ui_tooltip = "Half: input is squeezed, each eye fills its half of the frame completely.\n"
	             "Full: input has correct aspect ratio per eye, with black bars on the sides (SBS) or top/bottom (TaB).";
	ui_category = "Stereoscopic Conversion";
> = 0;

uniform int Output_Half_Full <
	ui_type = "combo";
	ui_items = "Half\0Full\0";
	ui_label = "Output Half/Full";
	ui_tooltip = "Half: output is squeezed, each eye fills its half of the frame completely.\n"
	             "Full: output has correct aspect ratio per eye, with black bars on the sides (SBS) or top/bottom (TaB).";
	ui_category = "Stereoscopic Conversion";
> = 0;

uniform float Full_Bar_Size <
	ui_type = "drag";
	ui_min = 0.0; ui_max = 0.4;
	ui_step = 0.001;
	ui_label = "Full Format Bar Size";
	ui_tooltip = "Size of the black bars as a fraction of each eye half, for Full SBS or Full TaB input/output.\n"
	             "Only applies when Input or Output is set to Full.";
	ui_category = "Stereoscopic Conversion";
> = 0.25;

uniform int Stereoscopic_Mode_Input <
	ui_type = "combo";
	ui_items = "Off\0Side by Side\0Top and Bottom\0Line Interlaced\0Checkerboard 3D\0Frame Sequential\0Frame Packing\0";
...
float4 Left, Right;
	...
	{
		// Half SBS: each eye fills its half completely, no bars. Y unchanged.
		// Full SBS: each eye has letterbox bars top/bottom within its half.
		//   Image content lives in y[Bar, 1-Bar] of each eye half.
		//   Map output y[0,1] -> source y[Bar, 1-Bar]: srcY = y * (1 - 2*Bar) + Bar
		float srcY = (Input_Half_Full == 1)
		           ? texcoord.y * (1.0 - 2.0 * Full_Bar_Size) + Full_Bar_Size
		           : texcoord.y;
		Left  = BB_Texture(float2(texcoord.x * 0.5 + P,        srcY));
		Right = BB_Texture(float2(texcoord.x * 0.5 + 0.5 - P,  srcY));
	}
	else if(Stereoscopic_Mode_Input == 2) //TnB
	{
		// Half TaB: each eye fills its half completely, no bars. X unchanged.
		// Full TaB: each eye has pillarbox bars left/right within its half.
		//   Image content lives in x[Bar, 1-Bar] of each eye half.
		//   Map output x[0,1] -> source x[Bar, 1-Bar]: srcX = x * (1 - 2*Bar) + Bar
		float srcX = (Input_Half_Full == 1)
		           ? texcoord.x * (1.0 - 2.0 * Full_Bar_Size) + Full_Bar_Size
		           : texcoord.x;
		Left  = BB_Texture(float2(srcX + P, texcoord.y * 0.5));
		Right = BB_Texture(float2(srcX - P, texcoord.y * 0.5 + 0.5));
	}
...
float4 toElse(float2 texcoord)
{
	...
	if (Stereoscopic_Mode == 0) // SBS output
		{
			TCL.x = (texcoord.x * 2.0) + P;
			TCR.x = (texcoord.x * 2.0 - 1.0) - P;
			if (Output_Half_Full == 1) // Full out: add letterbox bars top/bottom
			{
				TCL.y = (texcoord.y - Full_Bar_Size) / (1.0 - 2.0 * Full_Bar_Size);
				TCR.y = (texcoord.y - Full_Bar_Size) / (1.0 - 2.0 * Full_Bar_Size);
			}
			else // Half out: no bars
			{
				TCL.y = texcoord.y;
				TCR.y = texcoord.y;
			}
		}
		else if(Stereoscopic_Mode == 1) // TaB output
		{
			TCL.y = (texcoord.y * 2.0);
			TCR.y = (texcoord.y * 2.0 - 1.0);
			if (Output_Half_Full == 1) // Full out: add pillarbox bars left/right
			{
				TCL.x = (texcoord.x - Full_Bar_Size) / (1.0 - 2.0 * Full_Bar_Size) + P;
				TCR.x = (texcoord.x - Full_Bar_Size) / (1.0 - 2.0 * Full_Bar_Size) - P;
			}
			else // Half out: no bars
			{
				TCL.x = texcoord.x + P;
